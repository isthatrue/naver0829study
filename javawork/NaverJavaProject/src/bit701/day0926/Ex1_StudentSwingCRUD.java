...
public class Ex1_StudentSwingCRUD extends JFrame {
	...
	private void setDesign() {
		...
		btnAdd.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// db 에 insert 하는 메소드 호출
				insertStudent(name, phone, blood);
				// db 로부터 다시 데이터를 가져와서 출력
				studentSelectAll();
				// 입력값 초기화
				tfName.setText("");
				tfPhone.setText("");
		...
		btnDel.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (row == -1) {
					...
					// row 행의 0번열이 num 값
					String num = table.getValueAt(row, 0).toString();	// toString 사용 시 값이 없으면 NullPointerException 발생
					deleteStudent(num);
	...
	// db method
	// db 에서 전체 데이터를 가져와서 테이블에 출력하는 메소드
